(function () {
  ...
  const createStickySectionsRuntime = ({ reducedMotion, stickySections }) => {
    ...
    const getConvergenceVisual = section => section.scene.querySelector("[data-convergence-visual]");
    ...
    const setConvergenceVisualProgress = (section, progress) => {
      const visual = getConvergenceVisual(section);
      if (!visual) return;
    ...
    const updateStickySectionCounter = (counter, progress) => {
      counter.element.textContent = String(Math.round(counter.target * progress));
    };
    const updateStickySectionCard = (section, state, card, cardIndex, travel) => {
    ...
    const updateStickySectionLines = (section, cardProgresses) => {
    ...
    const updateStickySectionVisuals = (section, state) => {
    ...
    return {
      ...
      getCounterProgress,
      getStickySectionMetrics,
      getStickySectionState,
      resetStickySectionCards,
      setupTimelineRoutes,
      syncStickySectionMotionState,
      updateStickySectionCard,
      updateStickySectionCounter,
      updateStickySectionLines,
      updateStickySectionVisuals
    };
  };

  window.createStickySection = createStickySection;
  window.createStickySectionsRuntime = createStickySectionsRuntime;
})();
